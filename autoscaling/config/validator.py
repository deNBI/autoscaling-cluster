...
class ConfigValidator:
    ...
    def _validate_scaling(self, config: dict[str, Any]) -> None:
        ...
        valid_modes = ["basic", "approach", "adaptive", "sequence", "multi",
                       "max", "default", "flavor", "min", "reactive"]
        if active_mode and active_mode not in valid_modes:
            self.errors.append(
                f"Invalid active_mode '{active_mode}'. Must be one of: {valid_modes}"
            )
    ...
    def _validate_mode(self, mode_name: str, config: dict[str, Any]) -> None:
        ...
        if not isinstance(service_freq, int) or service_freq < 10:
            self.warnings.append(
                f"Mode '{mode_name}': service_frequency should be at least 10 seconds"
            )
        ...
        if not isinstance(scale_force, (int, float)) or not 0.0 <= scale_force <= 1.0:
            self.errors.append(
                f"Mode '{mode_name}': scale_force must be between 0.0 and 1.0"
            )
        ...
        if not isinstance(cool_down, int) or cool_down < 0:
            self.errors.append(
                f"Mode '{mode_name}': worker_cool_down must be a non-negative integer"
            )
        ...
        for limit_key in ["limit_memory", "limit_worker_starts", "limit_workers"]:
            ...
            if not isinstance(limit_value, int) or limit_value < 0:
                self.errors.append(
                    f"Mode '{mode_name}': {limit_key} must be a non-negative integer"
                )
        ...
        if not isinstance(smoothing, (int, float)) or not 0.0 <= smoothing <= 1.0:
            self.errors.append(
                f"Mode '{mode_name}': smoothing_coefficient must be between 0.0 and 1.0"
            )
        ...
        if flavor_depth not in valid_depths:
            self.errors.append(
                f"Mode '{mode_name}': flavor_depth must be one of {valid_depths}"
            )
    ...
    def _validate_time_values(self, config: dict[str, Any]) -> None:
        ...
        if time_range_max <= time_range_min:
            self.errors.append(
                "time_range_max must be greater than time_range_min"
            )

        if time_range_max < 60:
            self.warnings.append(
                "time_range_max is very low (less than 60 seconds)"
            )
