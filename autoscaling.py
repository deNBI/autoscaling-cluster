...
class TerminateProtected:
    ...
    def _handler(self, signum, frame):
        logging.error("Received SIGINT or SIGTERM! Finishing this code block, then exiting.")
        self.killed = True
...
class ScalingUp:
    def __init__(self, password, dummy_worker, cluster_data):
        self.password = password
        self.dummy_worker = dummy_worker
        if not cluster_data:  # speed up, if recent data available
    ...
    def replace_cidr_for_nsf_mount(self):
        ...
        with open(COMMON_CONFIGURATION_YML, 'r') as stream:
            ...
            try:
                ...
                print(exc)
                sys.exit(1)
        with open(COMMON_CONFIGURATION_YML, 'w') as f:
...
def get_dummy_worker(flavors_data):
    ...
    if flavors_data:
        ...
        for flavor_tmp in flavors_data:
            max_memory_tmp = convert_gb_to_mib(flavor_tmp['flavor']['ram_gib'])
            max_cpu_tmp = flavor_tmp['flavor']['vcpus']
            max_gpu_tmp = int(flavor_tmp['flavor']['gpu'])
            ephemeral_disk_tmp = convert_gb_to_mib(int(flavor_tmp['flavor']['ephemeral_disk']))
            if ephemeral_disk < ephemeral_disk_tmp:
                ephemeral_disk = ephemeral_disk_tmp
            if max_memory < max_memory_tmp:
                max_memory = max_memory_tmp
            if max_cpu < max_cpu_tmp:
                max_cpu = max_cpu_tmp
            if max_gpu < max_gpu_tmp:
                max_gpu = max_gpu_tmp
    else:
        logger.error("no flavor available")

    ephemerals = [{'size': ephemeral_disk, 'device': '/dev/vdb', 'mountpoint': '/mnt'}]
    dummy_worker = {'cores': max_cpu, 'ephemeral_disk': ephemeral_disk, 'ephemerals': ephemerals,
                    'hostname': NODE_DUMMY,
                    'ip': '0.0.0.4', 'memory': max_memory, 'status': 'ACTIVE', 'gpu': max_gpu}
...
def cluster_credit_usage():
    ...
    cluster_worker = get_cluster_workers_from_api()
    credit_sum = 0
    worker_credit_data = {}
    if not (cluster_worker is None):
        for cl in cluster_worker:
            if cl['status'] == WORKER_ACTIVE:
                credit_sum += cluster_worker_to_credit(flavors_data, cl)
                if cl['flavor'] in worker_credit_data:
                    worker_credit_data[cl['flavor']]['cnt'] += 1
                else:
                    worker_credit_data.update(
                        {cl['flavor']: {'cnt': 0, 'credit': cluster_worker_to_credit(flavors_data, cl)}})
    else:
        cluster_worker = []
    logger.info("current worker credit %s", credit_sum)
    return credit_sum, cluster_worker, worker_credit_data


def cluster_worker_to_credit(flavors_data, cluster_worker):
    """
    credit changes, new calculations
    'de.NBI highmem large',316, 335.2, 'ram_gib': 256.0
    'de.NBI large + ephemeral',47.2,47.2, 'ram': 64.0
    'de.NBI small + ephemeral', 12.8 - 12.8
    """

    fv = cluster_worker_to_flavor(flavors_data, cluster_worker)
    if fv:
        try:
            credit = float(fv['flavor']['credits_costs_per_hour'])
            return credit
        except KeyError:
...
def worker_to_cluster_data(key, cluster_worker):
    if cluster_worker:
        for cl in cluster_worker:
            if cl['hostname'] == key:
                return cl
...
def cluster_worker_to_flavor(flavors_data, cluster_worker):
    ...
    if cluster_worker:
        for fd in flavors_data:
            if cluster_worker['flavor'] == fd['flavor']['name']:
                return fd
...
def __csv_log_with_refresh_data(scale_ud, worker_change, reason):
    ...
    try:
        if scale_ud == 'C' and LOG_LEVEL == logging.DEBUG:
            # collect credit values only by regular data recall, time-consuming api call
            credit_, cluster_worker, worker_credit_data = cluster_credit_usage()
        else:
            credit_ = 0
            cluster_worker = []
            worker_credit_data = {}
        ...
        jobs_running = len(jobs_running_dict)
        worker_json, worker_count, worker_in_use, worker_drain, _ = receive_node_data_db(True)
        worker_free = (worker_count - len(worker_in_use))
        if worker_json is None:
            return
        __csv_log(jobs_running, jobs_pending, worker_count, len(worker_in_use), worker_free, scale_ud,
                  __count_current_cpu_mem_usage(worker_json, {**jobs_pending_dict, **jobs_running_dict}),
                  len(worker_drain),
                  worker_change, reason, credit_, cluster_worker, worker_credit_data)
...
def __csv_log(jobs_cnt_running, job_cnt_pending, worker_cnt, worker_cnt_use, worker_cnt_free, scale_ud,
              cpu_mem_sum, worker_drain_cnt, worker_change, reason, credit_, cluster_worker, worker_credit_data):
    """
    DEBUG
    ...
    :param credit_: current credit consumption
    :return:
    """
    ...
    w_data = [
        ...
        worker_drain_cnt,
        worker_change,
        reason, credit_, len(cluster_worker), worker_credit_data]
    ...
    if not os.path.exists(LOG_CSV) or os.path.getsize(LOG_CSV) == 0:
        ...
        header = [
            ...
            'w_change', 'reason', 'credit', 'cluster_worker', 'worker_credit_data'
        ]
...
def get_wrong_password_msg():
    logger.error(f"The password seems to be wrong. Please generate a new one "
...
def translate_metrics_to_flavor(cpu, mem, tmp_disk, flavors_data, available_check, quiet):
    """
    ...
    for fd in reversed(flavors_data):
        if fd['flavor']['ephemeral_disk'] == 0 and config_mode['flavor_ephemeral']:
            continue

        if cpu <= int(fd['flavor']['vcpus']) and mem <= int(fd['available_memory']) \
                and (int(tmp_disk) < int(fd['flavor']['tmp_disk']) or (
                int(tmp_disk) == 0 and int(fd['flavor']['tmp_disk'] >= 0))):
            ...
            if fd['usable_count'] > 0:
                ...
                if not quiet:
                    logger.info(
                        ...
                        fd['flavor']['name'], cpu, mem, tmp_disk)
                break
...
def __sort_job_priority(jobs_dict):
    ...
    priority_job_list = sorted(jobs_dict.items(),
                               key=lambda k: (k[1]['priority'], k[1]['req_mem'], k[1]['req_cpus'], k[1]['tmp_disk']),
                               reverse=True)
...
def __sort_job_by_resources(jobs_dict):
    ...
    """
    sorted_job_list = sorted(jobs_dict.items(), key=lambda k: (k[1]['req_mem'], k[1]['req_cpus'], k[1]['tmp_disk']),
                             reverse=True)
    ...
    if node_dict:
        ...
        for key, value in list(node_dict.items()):
            ...
            if 'worker' in key:
                ...
                del node_dict[key]
        if not quiet:
            logger.info("nodes: I found %d worker - %d allocated, %d drain, drain+idle %s", worker_count,
                        len(worker_in_use),
                        len(worker_drain), worker_drain_idle)
    else:
        if not quiet:
...
def worker_filter(cluster_worker):
    ignore_workers = config_data['ignore_workers']

    if ignore_workers:
        index = 0
        for key in cluster_worker:
            if key['hostname'] in ignore_workers:
                cluster_worker.pop(index)
    ...
    if jobs_dict:
        for key, value in list(jobs_dict.items()):
            if value['state'] != JOB_FINISHED:
                del jobs_dict[key]
        jobs_dict = dict(sorted(jobs_dict.items(), key=lambda k: (k[1]['end']),
                                reverse=False))
        for key, value in list(jobs_dict.items()):
            print(value['end'])
...
def print_job_data(job_data):
    if job_data:
        for key, value in list(job_data):
            logger.info("id: %s - jobname %s - req_mem %s - req_cpus %s - priority %s - tmp_disk %s - elapsed %s",
                        key, value['jobname'], value['req_mem'],
                        value['req_cpus'], value['priority'], value['tmp_disk'], value['elapsed'])
    else:
...
def get_cluster_data():
    ...
    try:
        ...
        if response.status_code == HTTP_CODE_OK:
            ...
            return res
        elif response.status_code == HTTP_CODE_UNAUTHORIZED:
            print(get_wrong_password_msg())
            logger.error(get_wrong_password_msg())
            sys.exit(1)
        elif response.status_code == HTTP_CODE_OUTDATED:
            automatic_update()
        else:
...
def get_cluster_workers(cluster_data):
    ...
    cluster_worker = None
    if cluster_data:
        cluster_worker = [data for data in cluster_data["active_worker"] if data is not None]

        for i, w_data in enumerate(cluster_worker):
            cluster_worker[i].update({'memory_usable': reduce_flavor_memory(convert_mib_to_gb(w_data['memory']))})
            if 'ephemerals' in w_data and w_data['ephemerals'] and 'size' in w_data['ephemerals'][0]:
                cluster_worker[i].update({'tmp_disk': w_data['ephemerals'][0]['size']})
            elif 'ephemeral_disk' in w_data:
                cluster_worker[i].update({'tmp_disk': w_data['ephemeral_disk']})
            else:
                cluster_worker[i].update({'tmp_disk': 0})
                logger.error("ephemeral missing in cluster worker data")
        worker_filter(cluster_worker)
...
def flavor_mod_gpu(flavors_data):
    ...
    for fd in flavors_data:
        ...
        if flavor_gpu == FLAVOR_GPU_ONLY and fd['flavor']['gpu'] == 0:
            removed_flavors.append(fd['flavor']['name'])
        # remove all gpu flavors
        elif flavor_gpu == FLAVOR_GPU_REMOVE and fd['flavor']['gpu'] != 0:
            removed_flavors.append(fd['flavor']['name'])
        else:
...
def get_usable_flavors(quiet, cut):
    ...
    try:
        ...
        if res.status_code == HTTP_CODE_OK:
            ...
            flavors_data = sorted(flavors_data, key=lambda k: (k['flavor']['credits_costs_per_hour'],
                                                               k['flavor']['ram_gib'], k['flavor']['vcpus'],
                                                               k['flavor']['ephemeral_disk']), reverse=True)
            ...
            for fd in flavors_data:
                ...
                if 'ephemeral_disk' in fd['flavor']:
                    fd['flavor']['tmp_disk'] = convert_gb_to_mib(fd['flavor']['ephemeral_disk'])
                else:
                ...
                if config_mode['limit_flavor_usage'] and fd['flavor']['type']['shortcut'] == FLAVOR_HIGH_MEM and \
                        fd['flavor']['name'] in config_mode['limit_flavor_usage']:
                    user_fv_limit = int(config_mode['limit_flavor_usage'][fd['flavor']['name']])
                    available_to_use = min(user_fv_limit - fd['used_count'],
                                           fd['real_available_count_openstack'], fd['available_count'])
                else:
                    available_to_use = min(fd['real_available_count_openstack'], fd['available_count'])
            ...
            logger.error(get_wrong_password_msg())
            sys.exit(1)
        elif res.status_code == HTTP_CODE_OUTDATED:
            logger.error("version missmatch error")
            automatic_update()
        else:
            logger.error("server error - unable to receive flavor data, code %s", res.status_code)
...
def __worker_states():
    ...
    cluster_worker = get_cluster_workers(cluster_data)
    ...
    if not (cluster_worker is None):
        for cl in cluster_worker:
            ...
            if NODE_DOWN in cl['status']:
                ...
                sys.exit(1)
            elif WORKER_ACTIVE == cl['status']:
                worker_active.append(cl['hostname'])
            else:
                worker_unknown.append(cl['hostname'])
    return worker_active, worker_unknown, worker_error, worker_down, cluster_worker, cluster_data
...
def check_workers(rescale, worker_count, dummy_worker):
    ...
    while not worker_ready:
        worker_active, worker_unknown, worker_error, worker_down, cluster_worker, cluster_data = __worker_states()

        if cluster_data is None:
            logger.error("unable to receive cluster data ... try again in %s seconds",
                         service_frequency * WAIT_CLUSTER_SCALING)
            time.sleep(service_frequency * WAIT_CLUSTER_SCALING)
            continue
        cluster_count_live = len(cluster_worker)
        ...
        if cluster_count_live < worker_count and WAIT_CLUSTER_SCALING > max_wait_cnt:
            logger.debug("increase wait time for cloud api %s, wait until %s workers are listed, current %s",
            ...
            cluster_scale_down_specific_self_check(worker_unknown + worker_error, rescale, dummy_worker)
            worker_count -= len(worker_unknown + worker_error)
            no_error = False
            __csv_log_with_refresh_data('E', len(worker_unknown + worker_error), '11')
            ...
            cluster_scale_down_specific_self_check(worker_down, rescale, dummy_worker)
            worker_count -= len(worker_down)
            no_error = False
            __csv_log_with_refresh_data('E', len(worker_down), '17')
            ...
            return worker_ready, no_error, cluster_data
        else:
...
def __generate_downscale_list(worker_data, count, jobs_dict):
    ...
    worker_remove = []
    worker_idle = []
    scale_down_frequency = int(config_mode['scale_down_frequency'])
    if count == 0:
    ...
    if scale_down_frequency > 0 and worker_idle:
...
def __drain_worker_check(worker_high, w_fv, cluster_worker):
    """
    ...
    wait_time = int(config_mode['drain_delay'])
    if wait_time < 0 and w_fv:
        ...
        for w_key, w_value in worker_json.items():
            ...
            if w_cd_tmp and w_cd_tmp['flavor'] == w_fv['flavor']['name']:
                ...
                logger.debug("active worker %s with flavor %s", w_key, w_cd_tmp['flavor'])
        workers_with_flavor_drain = [x for x in workers_with_flavor if x in worker_drain_list]
        workers_remaining = __division_round(len(workers_with_flavor) - abs(wait_time), 2) - (
            len(workers_with_flavor_drain))
        logger.debug("workers_remaining: %s, workers_with_flavor_drain: %s, worker_drain_list %s", workers_remaining,
                     workers_with_flavor_drain, worker_drain_list)
        if workers_remaining <= 0:
            logger.debug("skip drain for worker: %s, workers_with_flavor %s: %s, worker_drain_list %s",
                         worker_high,
                         len(workers_with_flavor),
                         workers_with_flavor, worker_drain_list)
            return False
    return True
...
def __scale_frequency_worker_check(worker_useless, worker_dict):
    ...
    Remove workers from the list if the elapsed time since last usage is lower than the 'scale_down_frequency'.
    :param worker_useless: worker list
    :param worker_dict: worker dictionary
    :return: worker list
    """
    jobs_history_dict_rev = sorted(
        list(receive_completed_job_data(1).items()), key=lambda k: (k[1]['end'], k[1]['req_mem']), reverse=True)
    time_now = __get_time()
    wait_time = int(config_mode['scale_down_frequency'])
...
def classify_jobs_to_flavors(job_priority, flavor_data):
    ...
    if flavor_max_depth == DEPTH_MULTI or flavor_max_depth == DEPTH_MULTI_SINGLE:
        depth_limit = len(flavor_data)
    ...
    for key, value in job_priority:
        if value['state'] == JOB_PENDING:
            flavor_tmp = translate_metrics_to_flavor(value['req_cpus'], value['req_mem'], value['tmp_disk'],
                                                     flavor_data, available_check, False)
    ...
    if missing_flavors:
        ...
        __csv_log_with_refresh_data('L', None, 0)
    ...
    for key, value in job_priority:
        if value['state'] == JOB_PENDING:
            ...
            if flavor_tmp is None:
                ...
                logger.debug("flavor_next %s %s, flavor_tmp %s %s",
                             flavor_next['flavor']['name'], flavor_next['cnt'],
                             flavor_tmp['flavor']['name'], flavor_tmp['cnt'])
...
def __compare_flavor_max(fv_max, fv_min):
    ...
    if fv_max['flavor']['ram_gib'] >= fv_min['flavor']['ram_gib'] and \
            fv_max['flavor']['vcpus'] >= fv_min['flavor']['vcpus']:
...
def __compare_worker_high_vs_flavor(fv_tmp, w_value):
    ...
    if fv_mem < w_mem_tmp and \
            fv_tmp['flavor']['vcpus'] <= w_value['cpus']:
        if config_mode['flavor_ephemeral']:
            return True
...
def __compare_worker_match_flavor(fv_tmp, w_value):
    ...
    w_mem_tmp = int(w_value['real_memory'])
    if (fv_mem == w_mem_tmp) and \
            int(fv_tmp['flavor']['vcpus']) >= int(w_value['cpus']):
        if config_mode['flavor_ephemeral']:
            return True
        if int(fv_tmp['flavor']['tmp_disk']) >= int(w_value['tmp_disk']):
            return True
...
def set_nodes_to_drain(jobs_pending_dict, worker_json, flavor_data, cluster_worker, dummy_worker):
    """
    ...
    for key, value in jobs_pending_dict.items():

        if value['state'] == JOB_PENDING:
            logger.debug("set_nodes_to_drain: job id %s, cpu %s, mem %s, disk %s, state %s",
                         value['jobname'], value['req_cpus'], value['req_mem'], value['tmp_disk'], value['state'])
            fv_tmp = translate_metrics_to_flavor(value['req_cpus'], value['req_mem'], value['tmp_disk'],
    ...
    logger.debug("set_nodes_to_drain max: %s, ram %s, cpu %s, disk %s",
                 fv_max['flavor']['name'], fv_max['flavor']['ram_gib'],
                 fv_max['flavor']['vcpus'], fv_max['flavor']['tmp_disk'])
    if not fv_max:
        return worker_data_changed
    for w_key, w_value in worker_json.items():
        if 'worker' in w_key:
            ...
            if config_mode['drain_only_hmf']:
                if w_fv:
                    if w_fv['flavor']['type']['shortcut'] != FLAVOR_HIGH_MEM:
                        logger.debug("no drain for %s flavor %s, worker %s, continue ... drain only %s",
                                     w_fv['flavor']['type']['shortcut'],
                                     w_fv['flavor']['name'], w_key, FLAVOR_HIGH_MEM)
                        continue
            ...
            # worker resources are over required resources
                ...
                # drain delay
                if __drain_worker_check(w_key, w_fv, cluster_worker):
    ...
    if nodes_resume > 0:
        __csv_log_with_refresh_data('X', nodes_resume, '1')
    if workers_drain:
        cluster_scale_down_specific_self_check(workers_drain, Rescale.INIT, dummy_worker)
...
def convert_to_large_flavor(job_priority, flavors_data, pending_cnt, flavor_min):
    ...
    while counter_index > 0:
        ...
        tmp_mem += sum_mem
        tmp_disk += sum_disk
        tmp_fv = translate_metrics_to_flavor(tmp_cpu, tmp_mem, tmp_disk, flavors_data, True, True)
        if tmp_fv is None:
            break
        if config_mode['large_flavors_except_hmf'] and tmp_fv['flavor']['type']['shortcut'] == FLAVOR_HIGH_MEM:
        ...
        counter_index -= 1
    if flavor_max:
        logger.debug("convert_to_large_flavor: max %s jobs with flavor %s", counter, flavor_max['flavor']['name'])
...
def __generate_upscale_limit(current_force, jobs_pending_flavor, worker_count, level):
    ...
    if worker_weight != 0:
        ...
        logger.debug("WORKER_WEIGHT: %s - worker_useful %s - force %s", worker_weight,
                     worker_count, force)
...
def __division_round(n, d):
    ...
    """
    try:
        return int((float(n) / float(d)) + 0.5)
    except ZeroDivisionError:
        return 0


def __division_round_up(n, d):
    """
    Calculate a division, if divided by zero, return zero.
    Round up/down decimal numbers.
    :param n: number
    ...
    """
    try:
        return math.ceil((float(n) / float(d)))
    except ZeroDivisionError:
        return 0


def __division_round_down(n, d):
    """
    Calculate a division, if divided by zero, return zero.
    :param n: number
    :param d: divisor
...
def __multiply(x, y):
    ...
    return round((float(x) * float(y)) + 0.5)


def __multiple_jobs_per_flavor(flavor_tmp,
                               average_job_resources_cpu, average_job_resources_memory, average_job_resources_tmp_disk):
    """
    ...
    """
    jobs_average_cpu = __division_round(flavor_tmp['flavor']['vcpus'], average_job_resources_cpu)
    jobs_average_memory = __division_round(flavor_tmp['available_memory'], average_job_resources_memory)
    logger.debug("fv_cpu %s, av_cpu %s, fv_mem %s, av_mem %s", flavor_tmp['flavor']['vcpus'], average_job_resources_cpu,
                 flavor_tmp['available_memory'], average_job_resources_memory)
    logger.debug("jobs_average_cpu %s, jobs_average_memory %s",
                 jobs_average_cpu, jobs_average_memory)
    if int(flavor_tmp['flavor']['tmp_disk']) != 0 and average_job_resources_tmp_disk != 0:
        ...
        jobs_average_tmp_disk = __division_round(flavor_tmp['flavor']['tmp_disk'], average_job_resources_tmp_disk)
...
def __current_job_lifetime(jobs_running_dict, worker_useful,
                           average_job_resources_cpu,
                           average_job_resources_memory,
                           average_job_resources_tmp_disk,
                           dict_db, flavor_next, worker_json, cluster_worker,
                           average_job_time_norm, worker_claimed, job_time_norm_sum,
                           jobs_pending_flavor):
    """
    ...
    :param job_time_norm_sum: merged job times from known jobs
    ...
    for key in worker_useful:
        if key in worker_claimed_keys:
            ...
            # not claimed and useful
            worker_useful_tmp.append(key)
    logger.debug("worker_useful_tmp %s, worker_useful_reclaim %s", worker_useful_tmp, worker_useful_reclaim)
    match_val = float(config_mode['job_time_threshold'])
    if dict_db and cluster_worker:
        logger.debug("worker_useful: %s, worker_claimed: %s, usable %s, av job norm %s", worker_useful, worker_claimed,
                     worker_useful_tmp, average_job_time_norm)

        # check none claimed workers
        for worker in worker_useful_tmp:
            ...
            for j_key, j_value in jobs_running_dict.items():
                if worker == j_value['nodes']:
                    ...
                    logger.debug("running job %s on compatible worker %s", j_value['jobname'], worker)
                    tmp_job_data = job_data_from_database(dict_db, flavor_next['flavor']['name'],
                                                          __clear_job_name(j_value), True)
                    ...
                    # include flavor times
                        ...
                        if config_mode['forecast_by_flavor_history']:
                            job_time_norm = flavor_data_from_database(dict_db, flavor_next['flavor']['name'])

                    if job_time_norm:
                        current_norm_runtime = __calc_job_time_norm(j_value['elapsed'], 1)
                        # expected runtime < 0, under specified configuration
                        expected_time_left = max(job_time_norm - current_norm_runtime, 0)
                        logger.debug(
                            "jobname %s, tmp_job_data %s, job_time_norm %s, current_norm_runtime %s, norm left %s",
                            j_value['jobname'], tmp_job_data, job_time_norm, current_norm_runtime, expected_time_left)
                        ...
                        logger.debug("worker %s, norm %s, job %s", worker, expected_time_left_sum, j_key)
            logger.debug("remaining worker %s resources cpu: %s, mem: %s, disk: %s", worker, w_free_cpu, w_free_mem,
                         w_free_disk)
            logger.debug("expected free worker %s resources cpu: %s, mem: %s, disk: %s", worker, tmp_cpu, tmp_mem,
                         tmp_disk)
            tmp_cpu += w_free_cpu
            tmp_mem += w_free_mem
            ...
            if isinstance(worker_data['free_mem'], int):
                logger.debug("free_mem %s, tmp_mem %s", worker_data['free_mem'], tmp_mem)
            ...
            if average_job_resources_tmp_disk != 0:
                simultaneous_jobs_per_worker = min(__division_round_down(tmp_cpu, average_job_resources_cpu),
            ...
            norm_left_tmp = __division_float(expected_time_left_sum, running_jobs_on_worker)
            logger.debug("norm_left_tmp %s, expected_time_left_sum %s, running_jobs_on_worker %s",
                         norm_left_tmp, expected_time_left_sum, running_jobs_on_worker)
            norm_free_tmp = max((match_val - norm_left_tmp), 0)
            # how many jobs we can expect to process on a single worker
            # add 0.1 for processing time
            jobs_per_timeslot_tmp = __division_round_up(norm_free_tmp + 0.1, average_job_time_norm)
            logger.debug("average_job_time_norm %s, jobs_per_timeslot_tmp %s",
                         average_job_time_norm, jobs_per_timeslot_tmp)
            jobs_worker_can_process = jobs_per_timeslot_tmp * simultaneous_jobs_per_worker

            logger.debug("worker %s can process %s jobs "
                         "with the normalized free time %s, todo %s "
                         "simultaneous jobs per worker possible: %s",
                         worker, jobs_worker_can_process, norm_free_tmp, norm_left_tmp, simultaneous_jobs_per_worker)

            if jobs_worker_can_process > 0:
                logger.debug(worker_claimed)
                logger.debug(type(worker_claimed))
                worker_claimed.update(
                    {worker: {'cpu': tmp_cpu - (average_job_resources_cpu * simultaneous_jobs_per_worker),
                              'mem': tmp_mem - (average_job_resources_memory * simultaneous_jobs_per_worker),
                              'disk': tmp_disk - (average_job_resources_tmp_disk * simultaneous_jobs_per_worker),
                              'norm': norm_free_tmp}})
                jobs_workers_can_process += jobs_worker_can_process
                norm_workers_free += norm_free_tmp
                logger.debug("claim worker %s", worker)
                if (job_time_norm_sum - norm_workers_free) < 0 and jobs_pending_flavor < jobs_workers_can_process:
                    break
                if jobs_pending_flavor < jobs_workers_can_process:
                    break
        logger.debug("jobs_workers_can_process %s ==== %s ====", jobs_workers_can_process, worker_claimed)
        ...
        for worker in worker_useful_reclaim:
            ...
            jobs_per_timeslot_tmp = max(__division_round(norm_free_tmp, average_job_time_norm), 1)
            jobs_worker_can_process = jobs_per_timeslot_tmp * simultaneous_jobs_per_worker
            if jobs_worker_can_process > 0:
                worker_claimed.update(
                    {worker: {'cpu': tmp_cpu - (average_job_resources_cpu * simultaneous_jobs_per_worker),
                              ...
                              'disk': tmp_disk - (average_job_resources_tmp_disk * simultaneous_jobs_per_worker),
                              'norm': norm_free_tmp}})
                logger.debug("reclaim update %s, %s", worker, worker_claimed[worker])
                jobs_workers_can_process += jobs_worker_can_process
                logger.debug(worker_claimed)
    ...
    logger.info("we can expect to run %s pending jobs on current allocated workers %s in time",
                jobs_workers_can_process_tmp, worker_claimed)
    logger.debug("free normalized time %s, workers claimed %s, job process %s, known jobs %s", norm_workers_free,
                 worker_claimed,
                 jobs_workers_can_process, jobs_pending_flavor)
    return jobs_workers_can_process_tmp


def __calculate_scale_up_data(flavor_job_list, jobs_pending_flavor, worker_count, worker_json, worker_drain, state,
                              flavors_data, flavor_next, level, worker_memory_usage, jobs_running_dict,
                              cluster_worker, flavors_started_cnt, level_pending, worker_claimed):
    """
    ...
    """
    logger.debug("--------------- %s : %s -----------------", inspect.stack()[0][3], flavor_next['flavor']['name'])
    need_workers = False
    large_flavors = config_mode['large_flavors']
    worker_capable, _, worker_useful, _ = __current_workers_capable(flavor_job_list, worker_json, None)
    ...
    logger.debug("worker_useful %s, worker_drain %s, worker_usable %s",
                 worker_useful, worker_drain, worker_usable)
    ...
    logger.info("calc worker for %s; pending %s jobs; worker active %s, capable %s %s, memory %s, need %s; state %s",
                flavor_next['flavor']['name'], jobs_pending_flavor, worker_count,
                worker_capable, worker_usable_cnt, worker_memory_usage, need_workers, state.name)
    ...
    if dict_db and forecast_by_flavor_history and not (flavor_depth == DEPTH_MAX_WORKER):
        ...
        if fv_name in dict_db['flavor_name']:
            job_time_norm_fv = min(float(dict_db['flavor_name'][fv_name]['fv_time_norm']), time_threshold)

    upscale_limit = __generate_upscale_limit(current_force, jobs_pending_flavor, worker_usable_cnt, level)
    logger.debug("track upscale limit level %s: %s, flavor time: %s", level, upscale_limit, job_time_norm_fv)
    # memory limit pre-check
    if limit_memory != 0:
        if worker_memory_usage > convert_tb_to_mb(limit_memory):
            upscale_limit = 0
    logger.debug("track upscale limit level %s: %s, worker_memory_usage: %s - memory limit check", level, upscale_limit,
                 worker_memory_usage)
    ...
    if need_workers and upscale_limit == 0:
        upscale_limit = 1
        logger.debug("track upscale limit level %s: %s, worker capable: %s - force up", level, upscale_limit,
                     worker_capable)
    if upscale_limit > 0 or level_pending > 0:
        ...
        for key, value in flavor_job_list:
            if value['state'] == JOB_PENDING:
                ...
                if forecast_by_job_history:
                    ...
                    # it is updated regularly when started as a service
                    tmp_job_data = job_data_from_database(dict_db, flavor_next['flavor']['name'], job_name, True)
                    if tmp_job_data:
                        job_time_norm = min(__read_job_time(tmp_job_data), time_threshold)
                    else:
                        # new job
                        job_time_norm = None
                    if job_time_norm:
                        # merge normalized values from pending jobs
                        # calculate new workers with jobs pending flavor and summarize normalized job time
                        # by a time based normalized job value over threshold, treat as one new worker
                        logger.debug("job_time_norm: sum %s, norm %s, mode %s",
                                     job_time_norm_sum, job_time_norm, time_threshold)
        ...
        # generate average_job_time_norm
        if job_time_norm_fv:
            job_time_norm_unk = jobs_pending_similar_data_missing * job_time_norm_fv
        else:
            job_time_norm_unk = jobs_pending_similar_data_missing * time_threshold

        # use only known jobs at lifetime check
        average_job_time_norm = __division_float(job_time_norm_sum, job_time_norm_cnt)
        # convert to a higher flavor when useful
        # resources for job resources vs flavor comparison
        fv_high, jobs_per_fv_high, average_job_resources_cpu, average_job_resources_memory, \
            average_job_resources_tmp_disk = \
            convert_to_large_flavor(flavor_job_list, flavors_data, jobs_pending_flavor, flavor_next)
        ...
        if forecast_by_job_history or forecast_by_flavor_history:
            ...
            # summarize normalized worker values, predict worker count
            job_time_norm_sum = max(float(job_time_norm_sum), 0.1)
            # generate time based limit
            similar_job_worker_cnt = __division_round_down(job_time_norm_sum, time_threshold)
            # generate upscale limit for jobs without job or flavor history data
            # if active, include flavor based values in calculation
            similar_job_worker_cnt_unk = __generate_upscale_limit(current_force,
                                                                  jobs_pending_similar_data_missing,
                                                                  worker_usable_cnt, level)
            if job_time_norm_fv:
                job_time_norm_sum += job_time_norm_unk
                job_time_norm_cnt += jobs_pending_similar_data_missing
                similar_job_worker_cnt = __division_round_down(job_time_norm_sum, time_threshold)
                average_job_time_norm = __division_float(job_time_norm_sum, job_time_norm_cnt)
            else:
            ...
            logger.debug("track upscale limit level %s: %s, sum: %s - job time similar search", level, upscale_limit,
                         job_time_norm_sum)
        if forecast_by_job_history or forecast_by_flavor_history:
            # CHECK ACTIVE WORKERS
            # - running jobs will finish soon?
            # - how many jobs can run on this worker?
            # - how many jobs can be processed in one timeslot? (job_time_threshold)
            # - use average pending resources
            # - reduce number of pending jobs and upscale_limit by result
            if forecast_active_worker > 0:
                worker_active_weight = \
                    __current_job_lifetime(jobs_running_dict, worker_usable,
                                           average_job_resources_cpu,
                                           average_job_resources_memory,
                                           average_job_resources_tmp_disk,
                                           dict_db, flavor_next, worker_json, cluster_worker,
                                           average_job_time_norm, worker_claimed,
                                           job_time_norm_sum, job_time_norm_cnt)

                # reduce worker time count by active workers
                # job_time_norm_sum_tmp = job_time_norm_sum
                ...
                if upscale_limit_active < upscale_limit:
                    ...
                    logger.debug("track upscale limit level %s: %s, lifetime: - %s", level,
                                 upscale_limit, jobs_pending_flavor)

                # reduce job stats by lifetime result
                if worker_active_weight > 0:
                    job_time_norm_cnt_tmp = upscale_limit_active * __division_float(job_time_norm_cnt,
                                                                                    jobs_pending_flavor)
                    job_time_norm_sum_tmp = job_time_norm_cnt_tmp * __division_float(job_time_norm_sum,
                                                                                     job_time_norm_cnt)
                    job_time_norm_sum = max(job_time_norm_sum_tmp, 0.1)
                    # generate time based limit update
                    similar_job_worker_cnt = __division_round(job_time_norm_sum, time_threshold)

                    # include if flavor data not available
                    if not job_time_norm_fv:
                        similar_job_worker_cnt += similar_job_worker_cnt_unk
                    ...
                    logger.debug("upscale_limit %s similar_job_worker_cnt %s", upscale_limit,
        ...
        if flavor_default:
            ...
            if fv_fix:
                if fv_fix['flavor']['ram_gib'] >= flavor_tmp['flavor']['ram_gib'] and \
        ...
        average_jobs_per_flavor = __multiple_jobs_per_flavor(flavor_tmp,
                                                             average_job_resources_cpu,
                                                             average_job_resources_memory,
                                                             average_job_resources_tmp_disk)
        # auto activate large flavors
        if auto_activate_large_flavors != 0 and \
                __division_round(upscale_limit, average_jobs_per_flavor) >= auto_activate_large_flavors:
            large_flavors = True
            logger.debug("auto activate high worker")
        # switch to higher flavor
        if large_flavors and fv_high:
            logger.debug("high_flavor found %s cnt %s with %s jobs per flavor, minimum flavor was %s cnt %s",
                         fv_high['flavor']['name'], fv_high['cnt'], jobs_per_fv_high,
                         flavor_tmp['flavor']['name'], flavor_tmp['cnt'])
        ...
        if average_jobs_per_flavor > 1:
            ...
            if upscale_limit_tmp < upscale_limit:
                logger.debug(
                    ...
                    average_jobs_per_flavor, upscale_limit, upscale_limit_tmp)
                upscale_limit = upscale_limit_tmp
        logger.debug("track upscale limit level %s: %s, jobs per flavor: %s", level,
                     upscale_limit,
        ...
        logger.debug("track upscale limit level %s: %s,  memory limit second check", level, upscale_limit)
        ...
        if limit_memory != 0:
            if worker_count >= limit_memory:
                ...
                logger.debug("limit workers: up %s, count %s, limit %s, reduced %s",
                             upscale_limit, worker_count, limit_memory,
                             limit_memory - worker_count)
                upscale_limit = limit_memory - worker_count
        logger.debug("track upscale limit level %s: %s  - limit workers", level, upscale_limit)
        ...
        if need_workers and upscale_limit < 1:
            ...
            upscale_limit = 1
        logger.debug("track upscale limit level %s: %s  - need workers", level, upscale_limit)
        ...
        if upscale_limit > flavor_count_available:
            logger.debug("track upscale limit level %s: %s  - reduce worker, available flavor %s ", level,
                         upscale_limit,
                         flavor_count_available)
            upscale_limit = flavor_count_available
            logger.info("insufficient resources")
            __csv_log_with_refresh_data('L', flavor_next['flavor']['name'], 0)
        # limit worker starts
    ...
    if upscale_limit <= 0 or not flavor_tmp:
        ...
        logger.info("calculated upscale_limit is 0 or missing flavor '%s', skip",
...
def __calc_job_time_norm(job_time_sum, job_num):
    ...
    try:
        ...
        if job_num > 0 and job_time_sum > 1:
            ...
            norm = float(job_time_average - time_range_min) / float(time_range_max - time_range_min)
...
def __multiscale_scale_down(scale_state, worker_json, worker_count, worker_free, jobs_pending_dict, dummy_worker):
    """
    ...
    """
    worker_down_cnt = __calculate_scale_down_value(worker_count, worker_free, scale_state)
    # maximum worker count to scale down

    if scale_state == ScaleState.DELAY:
        scale_state = ScaleState.DOWN
        time.sleep(int(config_mode['scale_delay']))  # wait and recheck slurm values after DELAY
    elif scale_state == ScaleState.DOWN:
        cluster_scale_down_specific(worker_json, worker_down_cnt, Rescale.CHECK, jobs_pending_dict, dummy_worker)
...
def __compare_custer_node_workers(cluster_workers, worker_json):
    ...
    worker_missing = []
    try:
        if not (cluster_workers is None):
            for cl in cluster_workers:
                cluster_worker_list.append(cl['hostname'])
            logger.debug("cluster_workers %s, worker_json %s", cluster_worker_list, worker_json.keys())
            worker_missing = list(set(cluster_worker_list).difference(list(worker_json.keys())))
            if worker_missing:
                logger.debug("found missing workers in node data %s", worker_missing)
    except AttributeError:
        logger.debug("received broken data, cluster_workers: %s, worker_json: %s", cluster_workers, worker_json)
    return worker_missing
...
def __scale_down_error_workers(worker_err_list, dummy_worker):
    ...
    if worker_err_list:
        logger.error("found %s broken workers on cluster, no state change %s in %s", len(worker_err_list),
                     worker_err_list, config_mode['scale_delay'])
        time.sleep(WAIT_CLUSTER_SCALING)
        cluster_scale_down_specific_self_check(worker_err_list, Rescale.CHECK, dummy_worker)
        __csv_log_with_refresh_data('E', '0', '0')


def __verify_cluster_workers(cluster_worker, worker_json, dummy_worker):
    """
    ...
    :param cluster_worker: cluster data from first call
    :param worker_json: worker information as json dictionary object
    :param dummy_worker: dummy worker data
...
def multiscale(flavor_data, dummy_worker):
    ...
    while state != ScaleState.SKIP and state != ScaleState.DONE:
        jobs_pending_dict, jobs_running_dict = receive_job_data()
        jobs_pending = len(jobs_pending_dict)
        worker_json, worker_count, worker_in_use_list, worker_drain, _ = receive_node_data_db(True)
        ...
        if state == ScaleState.DELAY:
            ...
            cluster_worker = __verify_cluster_workers(get_cluster_workers(cluster_data), worker_json, dummy_worker)
            if cluster_worker is None:
            ...
            if drain_large_nodes and jobs_pending > 0 and not drained:
                if set_nodes_to_drain(jobs_pending_dict, worker_json, flavor_data, cluster_worker, dummy_worker):
        ...
        logger.info("worker_count: %s worker_in_use: %s worker_idle: %s jobs_pending: %s", worker_count,
        ...
        # SCALE DOWN
            ...
            state = __multiscale_scale_down(state, worker_copy, worker_count, worker_free, jobs_pending_dict,
                                            dummy_worker)
        ...
        # SCALE DOWN and UP
            ...
            if state == ScaleState.DOWN_UP:
                ...
                scale_down_value = __calculate_scale_down_value(worker_count, worker_free, state)
                changed_data = cluster_scale_down_specific(worker_copy,
                                                           scale_down_value,
                                                           Rescale.NONE, jobs_pending_dict, dummy_worker)
                if not changed_data:
                    scale_down_value = 0
                logger.debug("scale_down_value %s, changed_data %s", scale_down_value, changed_data)
        ...
        # SCALE UP
            ...
            if state == ScaleState.DELAY:
                ...
                cluster_scale_up(jobs_pending_dict, jobs_running_dict, worker_count, worker_json, worker_drain, state,
                                 flavor_data, cluster_worker, dummy_worker)
                state = ScaleState.DONE
            elif state == ScaleState.FORCE_UP:
                logger.debug("---- SCALE UP - force scale up ----")
                if not cluster_scale_up(jobs_pending_dict, jobs_running_dict, worker_count - scale_down_value,
                                        worker_json, worker_drain, state,
                                        flavor_data, cluster_worker, dummy_worker) and changed_data:
...
def cloud_api(portal_url_scale, worker_data):
    ...
    try:
        ...
        if e.response.status_code == HTTP_CODE_OUTDATED:
            ...
            __sleep_on_server_error()# keep running on error - probably a temporary error, cloud services are currently under maintenance
...
def cluster_scale_up(jobs_pending_dict, jobs_running_dict, worker_count, worker_json, worker_drain, state, flavor_data,
                     cluster_worker, dummy_worker):
    """
    ...
    while flavor_col_len > flavor_index_cnt:  # and not data
        ...
        if level_sup and not skip_starts:
            ...
            time.sleep(WAIT_CLUSTER_SCALING)
            flavor_data = get_usable_flavors(True, True)  # check if available flavor count changed
            level_sup = False

        data_tmp, worker_memory_usage_tmp = __calculate_scale_up_data(flavor_job_list,
                                                                      jobs_pending_flavor,
                                                                      worker_count,
                                                                      worker_json, worker_drain,
                                                                      state,
                                                                      flavor_data,
                                                                      flavor_next,
                                                                      flavor_index_cnt,
        ...
        if data_tmp:
            ...
            if not skip_starts:
                if cloud_api(get_url_scale_up(), data_tmp):
                    ...
                    logger.debug("starting workers via API failed for level %s", flavor_index_cnt)
        logger.debug("flavor data generation for future jobs level: %s data: %s claimed %s", flavor_index_cnt, data_tmp,
                     worker_claimed)
        flavor_index_cnt += 1
    time_now = __get_time()
    ...
    if upscale_cnt > 0:
        ...
        logger.info("scale up started %s worker with %s different flavor in %s seconds",
...
def cluster_scale_down_specific_hostnames(hostnames, rescale, dummy_worker):
    ...
    return cluster_scale_down_specific_hostnames_list(worker_hostnames, rescale, dummy_worker)
...
def cluster_scale_down_specific_self_check(worker_hostnames, rescale, dummy_worker):
    ...
    return cluster_scale_down_specific_hostnames_list(scale_down_list, rescale, dummy_worker)
...
def cluster_scale_down_specific(worker_json, worker_num, rescale, jobs_dict, dummy_worker):
    """
...
def __cluster_scale_down_complete():
    ...
    return cluster_scale_down_specific(worker_json, worker_count, Rescale.CHECK, None, None)
...
def __cluster_shut_down():
    ...
    if not (cluster_worker is None):
        ...
        if worker_list:
            ...
            time.sleep(WAIT_CLUSTER_SCALING)
            return cluster_scale_down_specific_hostnames_list(worker_list, Rescale.CHECK, None)
    return False
...
def rescale_cluster(worker_count, dummy_worker):
    ...
    worker_ready, no_error_scale, cluster_data = check_workers(Rescale.NONE, worker_count, dummy_worker)
    ...
    if not rescale_success:
        ...
        worker_err_list = __check_cluster_node_workers(get_cluster_workers_from_api(), worker_json)
        if worker_err_list:
            cluster_scale_down_specific_self_check(worker_err_list, Rescale.NONE, dummy_worker)
            __csv_log_with_refresh_data('E', '0', '20')
...
def create_pid_file():
    ...
    if os.path.isfile(pidfile_):
        logger.critical("please terminate the running autoscaling process,\npid file already exist %s, exit\n",
                        pidfile_)
        sys.exit(1)
...
def __cluster_scale_up_choice():
            ...
            try:
                ...
                if len(flavors_data) > flavor_num:
                    ...
                    if flavor_available_cnt >= upscale_num:
                        ...
                        time.sleep(WAIT_CLUSTER_SCALING)
                        __cluster_scale_up_specific(flavor_name, upscale_num, False, None)
    ...
    if flavor_names:
        ...
        rescale_time = __get_time() - time_now
        logger.debug("scale up by choice success: flavors %s, %s seconds", flavor_names, rescale_time)
...
def __cluster_scale_down_choice_batch():
    ...
    if search_batch:
        ...
        return cluster_scale_down_specific_hostnames_list(scale_down_list, Rescale.CHECK, None)
    return False
    ...
    if fv_name in dict_db['flavor_name']:
        for current_job in dict_db['flavor_name'][fv_name]['similar_data']:
            ...
            if diff_match > job_match_value:
                # found
                logger.debug("diff_match %s, job_name %s current_job %s", diff_match, job_name, current_job)
    ...
    if multi_flavor:
        for flavor_tmp in dict_db['flavor_name']:
            ...
            for current_job in dict_db['flavor_name'][flavor_tmp]['similar_data']:
                diff_match = difflib.SequenceMatcher(None, job_name, current_job).ratio()
                if diff_match > job_match_value:
                    logger.debug("diff_match %s, job_name %s current_job %s other flavor %s",
                                 diff_match, job_name, current_job, flavor_tmp)
...
def create_database(flavor_data):
    ...
    if flavor_data is None:
        ...
        return None
    time_start = __get_time() - __get_history_recall() * 86400  # config_data['history_init']
...
def smooth_flavor_time(dict_db, current_flavor, smoothing_coefficient):
    ...
    :return: f_(t+1)
    """
    f_t1 = smooth_time(dict_db['flavor_name'][current_flavor]['fv_time_norm'],
...
def update_database(flavor_data):
    ...
    jobs_dict_new = receive_completed_job_data(1 + __division_round(current_time - last_update_time, 86400))
    ...
    for key, value in jobs_dict_new.items():
        if value['state'] != JOB_FINISHED or int(value['end']) < last_update_time or value['elapsed'] < 0:
            continue
        job_name = __clear_job_name(value)
        v_tmp = translate_metrics_to_flavor(value['req_cpus'], value['req_mem'], value['tmp_disk'],
                                            flavor_data, False, True)
        job_time_norm_tmp = __calc_job_time_norm(
            value['elapsed'],
            1
        )
        if not (v_tmp is None):
            ...
            if fv_name not in dict_db['flavor_name']:
                ...
                dict_db['flavor_name'].update(
                    {fv_name: {'fv_time_norm': job_time_norm_tmp,
                ...
                if smoothing_coefficient != 0:
                    fv_norm = smooth_flavor_time(dict_db, fv_name, smoothing_coefficient)
                else:
            ...
            for current_job in dict_db['flavor_name'][fv_name]['similar_data']:
                ...
                if diff_match > job_match_value:
                    found = True
                    job_cnt = dict_db['flavor_name'][fv_name]['similar_data'][current_job]['job_cnt'] + 1
                    job_sum = dict_db['flavor_name'][fv_name]['similar_data'][current_job]['job_sum'] + value[
                        'elapsed']
                    if smoothing_coefficient != 0:
                        job_norm = smooth_time(
                            dict_db['flavor_name'][fv_name]['similar_data'][current_job]['job_norm'],
                            value['elapsed'], smoothing_coefficient)
                    else:
                        job_norm = __calc_job_time_norm(
                            job_sum, job_cnt)
            ...
            if not found:
                ...
                dict_db['flavor_name'][fv_name]['similar_data'].update(
                    {job_name: {'job_norm': job_time_norm_tmp, 'job_elapsed': value['elapsed'],
...
def __clear_job_name(job_values):
    ...
    if config_data['pattern_id'] and config_data['pattern_id'] in job_values and job_values[config_data['pattern_id']]:
...
def version_check(version):
    ...
    """
    if version != VERSION:
        logger.warning(OUTDATED_SCRIPT_MSG.format(SCRIPT_VERSION=VERSION, LATEST_VERSION=version))
        automatic_update()
...
def status_response_systemd_service():
    ...
    :return:
    """
    get_status = subprocess.Popen("systemctl is-active autoscaling.service", shell=True, stdout=subprocess.PIPE).stdout
...
def update_file(file_location, url, filename):
    ...
    :url: Complete link to the external source.
    :param filename: Filename.
    :return: updated (boolean)
    """
    time.sleep(WAIT_CLUSTER_SCALING)
    try:
        logger.debug("download new  ", filename)
        res = requests.get(url, allow_redirects=True)
        if res.status_code == HTTP_CODE_OK:
            open(file_location, 'wb').write(res.content)
            return True
        else:
            logger.error("unable to update autoscaling %s from %s, status: %s", filename, url, res.status_code)
...
def function_test():
    ...
    if not (fv is None):
        ...
        if not __cluster_scale_up_specific(fv[-1]['flavor']['name'], 1, True, None):
            logger.error("unable to receive flavor data")
            result_ = False
        else:
...
def visualize_cluster_data(time_range):
    ...
    if time_range:
        if ':' in time_range:
            ...
            start_time = calendar.timegm(
                datetime.datetime(start_time[0], start_time[1], start_time[2], start_time[3], 0, 0).timetuple())
            stop_time = list(map(int, time_range[1].split('-')))
            stop_time = calendar.timegm(
                datetime.datetime(stop_time[0], stop_time[1], stop_time[2], stop_time[3], 0, 0).timetuple())
            print('requested: \nstart_time ', start_time,
                  time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(start_time)))
            print('stop_time ', stop_time, time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(stop_time)))
            print('available: \nstart_time', time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(my_data.time[0])))
            print('stop_time', time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(my_data.time[my_data.index[-1]])))
    ...
    for col in ['alloc', 'alloc_drain', 'mix', 'mix_drain', 'idle', 'idle_drain']:
        if not df[col].eq(0.0).all(axis=0):
            ax.bar(df['date'], df[col], bottom=bottom, width=widths, align='edge', alpha=0.4, label=names[cnt],
    ...
    if not df_jy.empty:
        plt.plot(df['date'], df_jy['job_alloc'], linestyle='solid', color='blue', alpha=0.4, label='allocated by jobs')
    if not dfmark_a.empty:
        plt.plot(dfmark_a['date'], dfmark_a['scale'], marker='p', linestyle='None', label='rescale', color='blue',
                 alpha=0.4,
                 markersize=4)
    if not dfmark_u.empty:
        plt.plot(dfmark_u['date'], dfmark_u['scale'], marker='^', linestyle='None', label='scale-up', color='green',
                 alpha=0.4,
                 markersize=4)
    if not dfmark_d.empty:
        plt.plot(dfmark_d['date'], dfmark_d['scale'], marker='v', linestyle='None', label='scale-down', color='orange',
                 alpha=0.4, markersize=4)
    if not dfmark_m.empty:
        plt.plot(dfmark_m['date'], dfmark_m['scale'], marker='1', linestyle='None', label='autoscale', color='black',
                 alpha=0.4,
                 markersize=4)
    if not dfmark_e.empty:
        plt.plot(dfmark_e['date'], dfmark_e['scale'], marker='X', linestyle='None', label='error', color='red',
                 alpha=0.4,
                 markersize=4)
    if not dfmark_y.empty:
        plt.plot(dfmark_y['date'], dfmark_y['scale'], marker='d', linestyle='None', label='scale-down drain',
                 color='blue',
                 alpha=0.4,
                 markersize=4)
    if not dfmark_x.empty:
        plt.plot(dfmark_x['date'], dfmark_x['scale'], marker='o', linestyle='None', label='reactivate worker',
                 color='orange',
                 alpha=0.4,
                 markersize=4)
    if not dfmark_i.empty:
        plt.plot(dfmark_i['date'], dfmark_i['scale'], marker='>', linestyle='None', label='scale-up (api)',
                 color='green',
                 alpha=0.4, markersize=4)
    if not dfmark_l.empty:
        plt.plot(dfmark_l['date'], dfmark_l['scale'], marker='D', linestyle='None', label='insufficient resources',
                 color='red',
                 alpha=0.4, markersize=4)
    ...
    ax.set_title(
        'Setting: ' + str(config_data['active_mode']) +
        ', force: ' + str(config_mode['scale_force']) + ', time: ' + str(config_mode['job_time_threshold']) +
        ', max: ' + str(config_data['time_range_max']) + ', freq: ' + str(config_mode['scale_down_frequency']),
        fontsize=15)
    ...
    for col in ['job_cnt_pending']:
        ax.bar(df['date'], df[col], width=widths, align='edge', alpha=0.4, label=names[cnt])
        bottom += df[col]
        cnt += 1
    ax.set_xlabel('time')
    ...
    try:
        ...
        for mode in yaml_config['scaling']['mode']:
            ...
            if 'info' in yaml_config['scaling']['mode'][mode]:
                ...
                print("id: {0:<2d}, {1:<20s}: {2:>1s}".format(modes_available.index(mode), mode, info))
            else:
                print("id: {0:<2d}, {1:<20s}".format(modes_available.index(mode), mode))

        print("current active mode: id {i}, {m}".format(i=modes_available.index(mode_active), m=mode_active))
...
if __name__ == '__main__':
    ...
    # stage 2, commands without service interruption and scheduler interface
    ...
    if len(sys.argv) == 2:
        if sys.argv[1] in ["-l", "--l", "-log", "--log"]:
            ...
            fv_data = get_usable_flavors(False, True)
            if fv_data and LOG_LEVEL == logging.DEBUG:
                print(pformat(fv_data))
                print(pformat(get_dummy_worker(fv_data)))
            sys.exit(0)
        ...
        if len(sys.argv) > 1:
            ...
            if len(sys.argv) == 2:
                if arg in ["-su", "--su", "-scaleup", "--scaleup"]:
                    ...
                    logger.warning("warning: scale down in %s sec ...", WAIT_CLUSTER_SCALING)
                    time.sleep(WAIT_CLUSTER_SCALING)
                    ...
                    __verify_cluster_workers(get_cluster_workers_from_api(), worker_j, None)
                ...
                if arg in ["-sus", "--sus", "-scaleupspecific", "--scaleupspecific"]:
                    ...
                    if sys.argv[3].isnumeric():
                        __cluster_scale_up_specific(sys.argv[2], int(sys.argv[3]), True, None)
        else:
