...
SCALE_FORCE_JOB_WEIGHT = 0.05  # similar job search, increases chance of new worker by job pending count
...
def __reduce_flavor_memory(mem_gb):
    ...
    """
    mem_mb = mem_gb * 1000
    return __reduce_flavor_memory_mb(mem_mb)


def __reduce_flavor_memory_mb(mem_mb):
    """
    receive raw memory in GB and reduce this to a usable value
    memory reduces to os requirements and other circumstances
    :param mem_mb: memory (raw mb value)
    :return: memory reduced value (mb)
    """
    gb2_mb = 1000  # 1024
    mem_gb = mem_mb / 1000
    ...
    # logger.debug("reduce_flavor_memory: mem_gb: %s , mem_mb: %s", mem_gb, mem_mb)

    if mem_low_border < mem_mb < mem_high_border:
        return int(mem_mb - (mem_gb * mem_multiplier))
    elif mem_mb > mem_high_border:
        return int(mem_mb - (mem_high_gb * mem_multiplier))
    return int(mem_mb - mem_low_reserve)
...
def __translate_cpu_mem_to_flavor(cpu, mem, flavors_data):
    ...
    for fd in reversed(flavors_data):
        ...
        logger.debug("flavor: \"%s\" -- ram: %s -- cpu: %s -- available: %sx ephemeral %s", fd['flavor']['name'],
...
def get_usable_flavors(cluster_id, cluster_pw):
    ...
    if res.status_code == 200:
        ...
        for fd in flavors_data:
            ...
            logger.info("flavor: %s - %s -- ram: %s GB -- cpu: %s -- available: %sx -- ephemeral_disk: %s ",
                        counter,
                        fd['flavor']['name'],
                        fd['flavor']['ram'],
                        fd['flavor']['vcpus'],
...
def check_workers(cluster_id, cluster_pw):
    ...
    while not worker_ready:
        ...
        if ((time.time() - start_time) > max_time) and worker_planned > 0:
            logger.error("workers are stuck as panned: %s", worker_err_list)
            ...
            if worker_error > 0 >= num_tries and num_tries < 15:
...
def check_workers_old(cluster_id, cluster_pw, second_test):
    """
    ...
    worker_error = False
    worker_err_list = ""
    worker_wait_on_err = 10  # try times

    logger.debug("check_workers - second call %s", second_test)

    while not worker_ready and worker_wait_on_err > 0:
        worker_ready = True
        worker_error = False
        if not SIMULATION:
            cluster_data = get_cluster_data(cluster_id, cluster_pw)
            if not (cluster_data is None):
                for cl in cluster_data:
                    ...
                    if 'DRAIN' in cl['status'] or 'DOWN' in cl['status']:
                        ...
                        sys.exit(1)
                    if cl['status'] == 'ERROR':
                        logger.debug("ERROR %s", cl['hostname'])
                        if worker_err_list != "":
                            worker_err_list = worker_err_list + "," + cl['hostname']
                        else:
                            worker_err_list = cl['hostname']
                        ...
                        worker_ready = False
        if worker_error:
            worker_wait_on_err -= 1
            time.sleep(WAIT_CLUSTER_SCALING_UP)
            logger.debug("worker_err_list %s", worker_err_list)
    ...
    if worker_error:
        if second_test:
            ...
            time.sleep(WAIT_CLUSTER_SCALING_UP)
            logger.debug("scale down error workers: %s", worker_err_list)
            cluster_scale_down_specific_hostnames(cluster_id, cluster_pw, worker_err_list, False)
            if not check_workers(cluster_id, cluster_pw):
                logger.debug("unable to delete worker: %s", worker_err_list)
                sys.exit(1)
            else:
                return True
...
def __generate_downscale_list(worker_data, count):
    ...
    # TODO workaround - required, if the most capable worker should remain active
    ...
    for j_key, j_value in job_priority:
        if j_value['state'] == 0:  # pending jobs
            ...
            for w_key, w_value in worker_json.items():
                ...
                if 'worker' in w_key:
                    w_mem_tmp = __reduce_flavor_memory_mb(w_value['real_memory'])
                    ...
                    if (j_value['req_mem'] <= w_mem_tmp) and (
                        ...
                        logger.debug("capable y - job mem %s worker mem %s, jcpu %s wcpu %s", j_value['req_mem'],
                                     str(w_mem_tmp), j_value['req_cpus'], w_value['cpus'])
                    else:
                        logger.debug("capable n - job mem %s worker mem %s, jcpu %s wcpu %s", j_value['req_mem'],
                                     str(w_mem_tmp), j_value['req_cpus'], w_value['cpus'])
            if not found_match:
                not_capable_counter += 1
                logger.debug("capable n - not_capable_counter", not_capable_counter)
    ...
    for key, value in job_priority:
        if value['state'] == 0:  # pending jobs
            if flavor_tmp_current is None:
                flavor_tmp_current = __translate_cpu_mem_to_flavor(value['req_cpus'], value['req_mem'], flavors_data)
            elif flavor_tmp_current == __translate_cpu_mem_to_flavor(value['req_cpus'], value['req_mem'], flavors_data):
                counter += 1
            else:
                return counter
    return counter
...
def __calculate_scale_up_data(cluster_id, cluster_pw, jobs_dict, jobs_pending, worker_count, worker_json, state):
    ...
    if not (job_times is None):
        upscale_limit = int(jobs_pending * (job_times + force) / 2)
        # logger.info("maybe we need %d new worker", upscale_limit)
    else:
        upscale_limit = int(jobs_pending * force)
...
def __verify_cluster_workers(cluster_id, cluster_pw, cluster_data_old, worker_json):
    """
...
def multiscale(cluster_id, cluster_pw):
    ...
    while state != ScaleState.SCALE_SKIP and state != ScaleState.SCALE_DONE:
        ...
        cluster_worker = __verify_cluster_workers(cluster_id, cluster_pw, cluster_worker, worker_json)
...
def cluster_scale_up(cluster_id, cluster_pw, jobs_dict, jobs_pending, worker_count, worker_json, state):
    """
...
def rescale_slurm_cluster(cluster_id, cluster_pw):
    ...
    if check_workers(cluster_id, cluster_pw):
        ...
        logger.debug("rescale cluster - done - %s, pw %s", scaling_file, cluster_pw)
        ...
        if len(sys.argv) > 1:
            ...
            if len(sys.argv) == 2:
                if arg in ["-c", "--c", "-clusterdata", "--clusterdata"]:
                    ...
                    time.sleep(WAIT_CLUSTER_SCALING_DOWN)
                    __verify_cluster_workers(cid, cpw, get_cluster_data(cid, cpw))
                    sys.exit(0)
                ...
                if arg in ["-m", "--m", "-mode", "--mode"]:
                    logger.warning("mode parameter: %s", arg2)
                    sc = __setting_overwrite(arg2)
